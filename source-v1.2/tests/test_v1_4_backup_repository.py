...
class V14BackupRepositoryTests(unittest.TestCase):
    ...
    def test_v14_identity_and_build_outputs_are_named_v14(self):
        ...
        standard = Path("FH6_Assistant_v1.5.spec").read_text(encoding="utf-8")
        portable = Path("FH6_Assistant_v1.5_portable.spec").read_text(encoding="utf-8")
        ...
        self.assertIn('WINDOW_TITLE = "FH6 Assistant v1.5"', identity)
        self.assertIn("FH6_Assistant_v1.5.spec", build_tool)
        self.assertIn("dist/FH6 Assistant v1.5.exe", package_tool)
        self.assertIn("FH6-Assistant-v1.5-Portable.zip", package_tool)
        self.assertIn("FH6-Assistant-v1.5-Full-Source.zip", package_tool)
        self.assertIn("name='FH6 Assistant v1.5'", standard)
        self.assertIn("name='FH6 Assistant v1.5 Portable'", portable)
        self.assertIn("ProductVersion', '1.5'", version)
        self.assertIn('app.setApplicationVersion("1.5")', app)
        self.assertIn('__version__ = "1.5"', package)
        self.assertIn(r"tools\build.py", build)
        self.assertNotIn("FH6_Assistant_v1.3.3.spec", build)
        self.assertTrue(readme.startswith("FH6 Assistant v1.5"))
