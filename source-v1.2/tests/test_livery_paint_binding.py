...
class LiveryPaintBindingTests(unittest.TestCase):
    def test_exact_kfps_x16_hash_custom_primary_applies_only_to_matching_primitive(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(diagnostic["manufacturer_global_gate"], "suppressed_by_explicit_custom_primary")
    ...
    def test_manufacturer_selector_applies_group_trailer_primary_when_entire_paint_state_is_uncustomised(self):
        with tempfile.TemporaryDirectory() as temp:
            glb = Path(temp) / "car.glb"
            ...
            self.assertFalse(diagnostic["global_custom_primary_active"])
            self.assertEqual(diagnostic["manufacturer_global_gate"], "manufacturer_primary_allowed")
            ...
            self.assertEqual(diagnostic["manufacturer_globally_suppressed_hashes"], [])
    ...
    def test_explicit_custom_primary_with_manufacturer_selector_uses_custom_and_never_resolves_factory_color(self):
        with tempfile.TemporaryDirectory() as temp:
            glb = Path(temp) / "car.glb"
            _write_glb(glb, [(_binding(BODY), 3)])
            scene = _scene([3])
            ...
            self.assertEqual(diagnostic["manufacturer_globally_suppressed_hashes"], [])
            self.assertEqual(diagnostic["manufacturer_selector_results"], [])

    def test_any_explicit_custom_primary_globally_suppresses_blank_record_manufacturer_selector(self):
        with tempfile.TemporaryDirectory() as temp:
            glb = Path(temp) / "car.glb"
            _write_glb(glb, [(_binding(BODY), 3), (_binding(HOOD), 2)])
            scene = _scene([3, 2])
            ...
            rendered, diagnostic = apply_exact_livery_paint_to_aux_stream(
                ...
                aux,
                _report([
                    _record(BODY, rgba=(180, 90, 45, 255), enabled=True, selector=0xFFFFFFFF),
                    _record(HOOD, rgba=(0, 0, 0, 0), enabled=False, selector=0),
                ]),
                _palette(primary=(0.95, 0.8, 0.1)),
            )
            expected = np.power(np.asarray([180, 90, 45], dtype=np.float32) / 255.0, 2.2)
            np.testing.assert_allclose(rendered[:3, 1:4], np.repeat(expected[None, :], 3, axis=0), atol=1e-6)
            np.testing.assert_array_equal(rendered[3:], aux[3:])
            self.assertTrue(diagnostic["global_custom_primary_active"])
            self.assertEqual(diagnostic["matched_custom_primitives"], 1)
            self.assertEqual(diagnostic["matched_manufacturer_primitives"], 0)
            self.assertEqual(
            ...
            self.assertEqual(diagnostic["manufacturer_selector_results"][0]["status"], "manufacturer_primary_suppressed_by_global_custom_paint")
            self.assertEqual(diagnostic["manufacturer_selector_results"][0]["primary_output"], "suppressed_global_custom_paint")
    ...
    def test_same_hash_on_non_paint_primitive_is_not_recolored(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            aux = np.asarray([[0.0, -1.0, -1.0, -1.0]] * 3, dtype=np.float32)
            rendered, diagnostic = apply_exact_livery_paint_to_aux_stream(glb, scene, aux, _report([_record(BODY)]))
            np.testing.assert_array_equal(rendered, aux)
            self.assertFalse(diagnostic["rendering_applied"])
