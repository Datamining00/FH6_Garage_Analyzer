...
def _zoom_controls(viewer: ZoomableImageView, *, full: bool) -> QWidget:
    ...
    if full:
        ...
        minus.setStyleSheet("QToolButton { background:white; color:#303341; border:1px solid #dfe1e8; border-radius:8px; font-size:16pt; font-weight:600; padding:0; }QToolButton:hover { border-color:#8c74ee; background:#f5f2ff; }")
        row.addWidget(minus)
    ...
    if full:
        ...
        plus.setStyleSheet("QToolButton { background:white; color:#303341; border:1px solid #dfe1e8; border-radius:8px; font-size:16pt; font-weight:600; padding:0; }QToolButton:hover { border-color:#8c74ee; background:#f5f2ff; }")
        row.addWidget(plus)
    row.addStretch(1)
    return holder
...
def _three_d_options() -> tuple[QWidget, dict[str, Any]]:
    ...
    row.addStretch(1)
    return holder, {"reset": reset, "eligibility": eligibility, "cleanup_c": cleanup_c}


def _show_preview_modes(window: Any, record: LiveryRecord | TuningRecord) -> None:
    image = _read_image(record.thumbnail_path)
    if image.isNull():
        QMessageBox.information(window, _txt("이미지 없음", "No image"), _txt("표시할 이미지를 찾을 수 없습니다.", "No preview image is available."))
        return
    ...
    dialog.resize(max(900, min(1500, int(available.width() * 0.92))), max(620, min(960, int(available.height() * 0.90))))
    ...
    three_d_message = QLabel(_txt("3D 모드는 처음 선택할 때만 준비됩니다.", "3D mode is prepared only when selected for the first time."))
    ...
    if not isinstance(record, LiveryRecord):
        ...
        mode_buttons[2].setToolTip(_txt("3D 리버리 보기는 리버리 항목에서만 사용됩니다.", "3D livery preview is available for livery items only."))
    ...
    hint = QLabel(_txt("마우스 휠: 확대/축소 · 드래그: 이동 · 더블클릭: 100%", "Mouse wheel: zoom · Drag: pan · Double-click: 100%"))
    ...
    def prepare_three_d() -> None:
        ...
        if not callable(callback):
            try:
                ...
                three_d_message.setText(_txt("3D 백엔드를 불러올 수 없습니다.\n", "Unable to load the 3D backend.\n") + f"{type(exc).__name__}: {exc}")
                return
        ...
        def invoke_backend() -> None:
            ...
            try:
                ...
                three_d_message.setText(_txt("3D 모델을 표시할 수 없습니다.\n", "Unable to display the 3D model.\n") + f"{type(exc).__name__}: {exc}")
    ...
    def switch_mode(index: int) -> None:
        ...
        if index == 0:
            hint.setText(_txt("마우스 휠: 확대/축소 · 드래그: 이동", "Mouse wheel: zoom · Drag: pan"))
        elif index == 1:
            hint.setText(_txt("마우스 휠: 확대/축소 · 드래그: 이동 · 더블클릭: 100%", "Mouse wheel: zoom · Drag: pan · Double-click: 100%"))
        else:
            hint.setText(_txt("좌클릭: 회전 · 우클릭: 이동 · 마우스 휠: 확대/축소", "Left-drag: rotate · Right-drag: pan · Mouse wheel: zoom"))
            prepare_three_d()
