...
from .carbin import parse_fh6_carbin
from .modelbin import MeshStructureRecord, parse_modelbin_mesh_structures
NEUTRAL_GEOMETRY_REVISION = 2
DRAW_EXTERIOR = 1
DRAW_COCKPIT = 2
DRAW_SHADOW = 4
DRAW_HOOD = 8
DRAW_WINDSHIELD_REFLECTION = 16
DRAW_DRIVERLESS_COCKPIT = 32
DRAW_WINDSHIELD_REFLECTION_DRIVERLESS = 64
DRAW_PROXY_LOD = 128
PRIMARY_DRAW_GROUPS = DRAW_EXTERIOR | DRAW_COCKPIT | DRAW_HOOD
ALTERNATE_DRAW_GROUPS = DRAW_WINDSHIELD_REFLECTION | DRAW_DRIVERLESS_COCKPIT | DRAW_WINDSHIELD_REFLECTION_DRIVERLESS | DRAW_PROXY_LOD
SUPPORT_PART_TYPES = frozenset({5, 6, 7})
...
class NeutralGeometryResult:
    ...
    def as_dict(self) -> dict:
        return {'status': self.status, 'revision': self.revision, 'mesh_count': self.mesh_count, 'ab_hidden_meshes': self.ab_hidden_meshes, 'a_presentation_hidden_meshes': self.a_presentation_hidden_meshes, 'b_support_hidden_meshes': self.b_support_hidden_meshes, 'c_candidate_meshes': self.c_candidate_meshes, 'wheelstyle_hidden_meshes': self.wheelstyle_hidden_meshes, 'extreme_thin_hidden_meshes': self.extreme_thin_hidden_meshes, 'mapped_render_meshes': self.mapped_render_meshes, 'unmapped_render_meshes': self.unmapped_render_meshes, 'carbin_instances_matched': self.carbin_instances_matched, 'carbin_instances_unmatched': self.carbin_instances_unmatched, 'policy': {'A': 'hide structurally presentation-only draw groups and validated shadow-only mesh passes', 'B': 'hide WheelStyle plus wheel-position-dependent support geometry from semantic converter/carbin structure', 'thin_geometry': 'hide extreme sheet/line-like auxiliary meshes using AABB ratios while preserving paint/glass and brakes', 'C': 'optional aggressive alternate-presentation cleanup candidate; default off', 'vehicle_specific_rules': False, 'filename_matching_for_visibility': False, 'material_name_matching_for_visibility': False}}
...
def _carbin_instance_key(model: dict, resolved_part_type: int) -> tuple[str, str, int, int, str]:
    return (_canonical_source(model.get('resource_path')), str(model.get('bone_name') or '').casefold(), int(model.get('bone_id', 0)), int(resolved_part_type), ','.join((_float_bits(value) for value in model.get('transform_matrix_row_major') or [])))
...
def _glb_instance_key(identity: object) -> tuple[str, str, int, int, str] | None:
    ...
    try:
        return (_canonical_source(source), bone.casefold(), int(bone_id), int(part_type), transform.upper())
...
def _is_extreme_thin_mesh(document: dict, mesh: dict, extras: dict, model_meta: dict | None) -> bool:
    ...
    if middle <= 1e-07:
        return True
    return short / middle <= THIN_RATIO_TO_MIDDLE_MAX and short / long <= THIN_RATIO_TO_LONG_MAX
...
def _load_carbin_metadata(archive: zipfile.ZipFile, carbin_entry: str) -> dict[tuple[str, str, int, int, str], dict]:
    ...
    def add_model(part: dict, model: dict) -> None:
        ...
        result[key] = {'part_type': resolved, 'part_type_name': str(part.get('resolved_part_type_name') or ''), 'assembly_name': str(model.get('assembly_name') or ''), 'draw_groups': int(model.get('draw_groups_raw', 0) or 0), 'is_interior_windshield': bool(model.get('is_interior_windshield', False))}
...
def annotate_neutral_geometry(glb_path: str | Path, converter_archive: str | Path, carbin_entry: str) -> NeutralGeometryResult:
    glb = Path(glb_path)
    ...
    with zipfile.ZipFile(archive_path, 'r') as archive:
        ...
        for (source, identity), glb_group in grouped.items():
            ...
            primary_records = [record for record in records_by_source.get(source, []) if record.lod_flags & 1]
            ...
            for group_index, (mesh_index, mesh) in enumerate(glb_group):
                ...
                if draw_groups & ALTERNATE_DRAW_GROUPS and record is not None and (not record.is_opaque) and (record.is_decal or record.is_transparent):
    ...
    return NeutralGeometryResult(status='annotated', revision=NEUTRAL_GEOMETRY_REVISION, mesh_count=len(meshes), ab_hidden_meshes=len(ab_hidden), a_presentation_hidden_meshes=a_hidden, b_support_hidden_meshes=b_hidden, c_candidate_meshes=c_candidates, wheelstyle_hidden_meshes=wheelstyle_hidden, extreme_thin_hidden_meshes=extreme_thin_hidden, mapped_render_meshes=mapped_render_meshes, unmapped_render_meshes=unmapped_render_meshes, carbin_instances_matched=carbin_instances_matched, carbin_instances_unmatched=carbin_instances_unmatched)
