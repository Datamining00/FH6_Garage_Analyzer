    ...
    def run(self) -> None:
        try:
            ...
            if game_root is None:
                raise RuntimeError('FH6 installation could not be located. Set FH6_GAME_DIR or FORZA_HORIZON_6_DIR if the game is installed in an unusual location.')
            ...
            if carbin_entry is None:
                raise RuntimeError(f'Car ID {self.car_id} has multiple ambiguous carbin scenes; automatic 3D preview was not attempted.')
...
class _SceneReloadWorker(QObject):
    ...
    def __init__(self, glb_path: str, textures: object, eligibility: str, cleanup_c: bool):
        ...
        self.cleanup_c = cleanup_c

    @Slot()
    def run(self) -> None:
        try:
            from .glb_parser import load_kfps_glb
            scene = load_kfps_glb(self.glb_path, self.textures, livery_uv_channel=3, livery_eligibility=self.eligibility, neutral_cleanup_c=self.cleanup_c)
...
def _prepare_preview_3d(window: Any, *, dialog: Any, record: Any, page: Any, layout: Any, message: QLabel, controls: dict[str, Any]) -> None:
    ...
    if not livery_path or not Path(livery_path).is_file() or car_id is None:
        message.setText(_txt('3D에 필요한 C_livery 또는 Car ID가 없습니다.', 'C_livery or Car ID required for 3D preview is unavailable.'))
        return
    state = {'viewer': None, 'glb_path': '', 'textures': None, 'loading': True, 'pending_reload': False}
    ...
    def show_error(text: str) -> None:
        state['loading'] = False
        message.setText(_txt('3D 모델을 표시할 수 없습니다.\n', 'Unable to display the 3D model.\n') + text)
        message.show()
        set_options_enabled(False)
    ...
    def reload_scene() -> None:
        ...
        if viewer is not None:
            viewer.hide()
        message.setParent(page)
        message.setText(_txt('3D 표시 옵션 적용 중...', 'Applying 3D display options...'))
        message.show()
        ...
        def reload_failed(text: str) -> None:
            show_error(text)
        ...
        _start_job(window, worker, finished=reloaded, failed=reload_failed)
...
def apply_preview_3d_backend_patch(MainWindow: Any) -> None:
    if getattr(MainWindow, '_fh6_v14_preview3d_backend_patched', False):
        return
    MainWindow._fh6_prepare_livery_3d_preview = _prepare_preview_3d
    MainWindow._fh6_v14_preview3d_backend_patched = True
