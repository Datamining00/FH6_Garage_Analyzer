...
class CarbinStructuralError(ValueError):
    pass
PART_NAMES: dict[int, str] = {0: 'CCarParts_Engine', 1: 'CCarParts_Drivetrain', 2: 'CCarParts_CarBody', 3: 'CCarParts_Motor', 4: 'CCarParts_Brakes', 5: 'CCarParts_SpringDamper', 6: 'CCarParts_AntiSwayFront', 7: 'CCarParts_AntiSwayRear', 8: 'CCarParts_TireCompound', 9: 'CCarParts_RearWing', 10: 'CCarParts_RimSizeFront', 11: 'CCarParts_RimSizeRear', 34: 'CCarParts_FrontBumper', 35: 'CCarParts_RearBumper', 36: 'CCarParts_Hood', 37: 'CCarParts_SideSkirts', 38: 'CCarParts_TireWidthFront', 39: 'CCarParts_TireWidthRear', 40: 'CCarParts_WeightReduction', 41: 'CCarParts_ChassisStiffness', 42: 'CCarParts_Ballast', 43: 'CCarParts_MotorParts', 44: 'CCarParts_WheelStyle', 45: 'CCarParts_Aspiration'}
...
class _Reader:
    ...
    def _take(self, size: int, context: str) -> bytes:
        if size < 0 or size > self.remaining():
            raise CarbinStructuralError(f'{context}: need {size} bytes at 0x{self.offset:X}, only {self.remaining()} remain')
        start = self.offset
        self.offset += size
...
def _parse_model(reader: _Reader, scene_version: int, context: str) -> dict:
    ...
    if model_version != 21:
        raise CarbinStructuralError(f'{context}: unsupported FH6 model version {model_version}; expected 21 for scene v7')
    ...
    for index in range(material_override_count):
        ...
        if value_length > _MAX_BLOB:
            raise CarbinStructuralError(f'{context} material override {index}: value length {value_length} exceeds {_MAX_BLOB}')
    ...
    for index in range(ao_count):
        ...
        ao: dict = {'version': ao_version, 'path': ao_path, 'part_type': ao_part_type, 'part_id': ao_part_id}
        if ao_version >= 2:
    ...
    return {'model_version': model_version, 'resource_path': path, 'transform_matrix_row_major': transform, 'lod_flags': lod_flags, 'bone_name': bone_name, 'bone_id': bone_id, 'snap_to_parent': snap_to_parent, 'draw_groups_raw': draw_groups, 'draw_groups': {'exterior': bool(draw_groups & 1), 'cockpit': bool(draw_groups & 2), 'shadow': bool(draw_groups & 4), 'hood': bool(draw_groups & 8), 'windshield_reflection': bool(draw_groups & 16), 'driverless_cockpit': bool(draw_groups & 32), 'windshield_reflection_driverless': bool(draw_groups & 64), 'proxy_lod': bool(draw_groups & 128)}, 'material_override_count': material_override_count, 'material_override_keys': material_override_keys, 'material_index_count': material_index_count, 'material_indexes': material_indexes, 'is_droppable': is_droppable, 'drop_value': drop_value, 'drop_part_id': drop_part_id, 'break_amount': break_amount, 'ao_map_info_count': ao_count, 'ao_map_infos': ao_maps, 'is_interior_windshield': is_interior_windshield, 'receives_impact_mask': receives_impact_mask, 'receives_splatter_mask': receives_splatter_mask, 'receives_damage': receives_damage, 'receives_dirt': receives_dirt, 'receives_oil': receives_oil, 'receives_rubber': receives_rubber, 'assembly_name': assembly_name, 'guid_v13': guid_v13, 'drop_guid_v14': drop_guid_v14, 'ao_map_info_id_v14': ao_map_info_id_v14, 'horizon_unk_v15': horizon_unk_v15, 'damage_guid_count': damage_guid_count, 'damage_guids': damage_guids, 'horizon_id': horizon_id, 'horizon_unk_v18': horizon_unk_v18, 'horizon_unk_v21_flag': horizon_unk_v21_flag, 'horizon_unk_v21_path': horizon_unk_v21_path}
...
def _part_header(raw_type: int, prefix_type: int | None, kind: str, index: int, version: int) -> dict:
    ...
    return {'kind': kind, 'part_index': index, 'part_version': version, 'prefix_part_type_raw': prefix_type, 'prefix_part_type_resolved': prefix_resolved, 'prefix_matches_payload': prefix_resolved is None or prefix_resolved == resolved, 'raw_part_type': raw_type, 'resolved_part_type': resolved, 'resolved_part_type_name': _part_name(resolved), 'is_tire_geometry_part': resolved in TIRE_GEOMETRY_PART_TYPES, 'is_rim_geometry_part': resolved in RIM_GEOMETRY_PART_TYPES}

def parse_fh6_carbin(data: bytes) -> dict:
    reader = _Reader(bytes(data))
    scene_version = reader.u16('scene version')
    if scene_version != 7:
        raise CarbinStructuralError(f'unsupported scene version {scene_version}; this FH6 diagnostic expects scene v7')
    ...
    for part_index in range(standard_count):
        ...
        item['models'] = [_parse_model(reader, scene_version, f'standard part {part_index} model {model_index}') for model_index in range(model_count)]
        if part_version >= 2:
    ...
    for part_index in range(upgradable_count):
        ...
        for upgrade_index in range(upgrades_count):
            ...
            upgrade = {'version': upgrade_version, 'level': reader.u8(f'upgrade {part_index}/{upgrade_index} level'), 'is_stock': bool(reader.u8(f'upgrade {part_index}/{upgrade_index} is stock')), 'part_id': reader.i32(f'upgrade {part_index}/{upgrade_index} part id'), 'car_body_id': reader.i32(f'upgrade {part_index}/{upgrade_index} car body id'), 'parent_is_stock': bool(reader.u8(f'upgrade {part_index}/{upgrade_index} parent stock'))}
            if upgrade_version < 3:
                legacy_model_count = reader.count(f'upgrade {part_index}/{upgrade_index} legacy model count')
                upgrade['legacy_models'] = [_parse_model(reader, scene_version, f'upgrade {part_index}/{upgrade_index} model {i}') for i in range(legacy_model_count)]
        ...
        if part_version >= 3:
            ...
            for model_index in range(shared_count):
                upgrade_id_count = reader.count(f'upgradable part {part_index} shared model {model_index} upgrade id count')
                upgrade_ids = [reader.i32(f'upgradable part {part_index} shared model {model_index} upgrade id {i}') for i in range(upgrade_id_count)]
                model = _parse_model(reader, scene_version, f'upgradable part {part_index} shared model {model_index}')
    ...
    return {'parser': 'fh6_structural_carbin_v1', 'reference': {'project': 'D3FEKT/ForzaTechStudio', 'commit': '4f373c5fb192551ce5249e320dd79b1399b693ca', 'license': 'MIT', 'classification_basis': 'numeric CCarParts enum and serialized carbin graph', 'vehicle_name_matching_used': False, 'resource_filename_matching_used_for_part_classification': False}, 'scene': {'scene_version': scene_version, 'expected_game': 'Forza Horizon 6', 'model_version_expected': 21, 'build_guid_hex': build_guid, 'build_strict': build_strict, 'ordinal': ordinal, 'media_name': media_name, 'skeleton_path': skeleton_path, 'lod_flags': lod_flags}, 'standard_part_count': len(standard_parts), 'standard_parts': standard_parts, 'upgradable_part_count': len(upgradable_parts), 'upgradable_parts': upgradable_parts, 'tire_geometry_part_type_values': sorted(TIRE_GEOMETRY_PART_TYPES), 'tire_related_part_count': len(tire_related), 'tire_related_parts': tire_related, 'rim_related_part_count': len(rim_related), 'rim_related_parts': rim_related, 'trailer': trailer}
