...
def mesh_family(name: str) -> str:
    """Normalize a Forza mesh LOD suffix without using vehicle/part-specific names."""
    value = (name or "").strip().casefold()
    return _LOD_SUFFIX_RE.sub("", value)


def parse_modelbin_mesh_lods(data: bytes) -> list[MeshLodRecord]:
    """Parse only Bundle/Mesh header metadata needed for near-LOD selection.

    This deliberately does not decode vertex/index payloads.  Offsets and the MeshBlob
    wire layout follow ForzaTechStudio's public Bundle.cs/BundleBlob.cs/MeshBlob.cs.
    """
...
def _patch_modelbin_near_lod(data: bytes) -> tuple[bytes, int, int, int, set[str]]:
    """Make KFPS bit0 selector equivalent to the actual LOD0 `(flags & 3) != 0` selector."""
    ...
    for record in records:
        ...
        if (flags & 0x02) and not (flags & 0x01):
            struct.pack_into("<H", mutable, record.lod_offset, flags | 0x01)
            changed += 1
...
def _filter_slod_to_unique_families(data: bytes, allowed_families: set[str]) -> tuple[bytes, int, set[str]]:
    """Keep only proven-unique near families visible to the converter's bit0 selector."""
    records = parse_modelbin_mesh_lods(data)
    mutable = bytearray(data)
    selected = 0
...
def _referenced_model_paths(
    carbin: bytes, archive_names: dict[str, str], model_code: str
) -> tuple[dict[str, str], tuple[str, ...], tuple[str, ...]]:
    """Resolve every unique modelbin reference found in the carbin.

    Returns (resolved_map, discovered_refs, unresolved_refs).  A caller must not
    treat a partial resolution as success: silently dropping a model reference can
    create a visually plausible but incomplete chassis.
    """
    output: dict[str, str] = {}
...
def _rewrite_selected_slod_paths(
    carbin: bytes, selected_entries: set[str], model_code: str
...
def discard_near_lod_archive(path: str | Path) -> None:
    """Delete a reproducible near-LOD derivative and its sidecar.

    This cache is intentionally transient; the expensive result worth keeping is
    the GLB, not a second copy of the source vehicle ZIP.
    """
    target = Path(path)
...
def cleanup_near_lod_derivatives(cache_root: str | Path, older_than_seconds: int = 24 * 60 * 60) -> int:
    """Remove only stale crash leftovers, never active cross-process derivatives."""
    import time
...
def prepare_near_lod_archive(
    ...
    """Create/reuse a LocalAppData-only archive whose near geometry is converter-safe.

    Source archive is opened read-only.  All mutation is applied to a derived ZIP below
    cache_root.  No game/save file is ever written.
    """
    ...
    with zipfile.ZipFile(source, "r") as src:
        ...
        # Raw carbin data can contain unselected upgrade/optional model paths. Do not
        # reject those here. The pinned scene parser later reports unresolved *active*
        # required instances separately from skipped optional instances; that semantic
        # result is the authoritative completeness check. We still record every raw
        # unresolved reference for diagnostics.
        ...
        for entry in referenced_entries:
            ...
            if "__slod" not in entry.casefold():
                ...
                for record in records:
                    if record.lod_flags & 0x03 and record.family:
                        normal_family_keys.add((parent, record.family))

        if unparsed:
            raise NearLodNormalizationError(
                "Near-LOD normalization could not structurally parse every referenced modelbin; "
                "refusing a partial chassis assembly. First unsupported entry: " + sorted(unparsed, key=str.casefold)[0]
            )

        # SLOD is a supplement only when it contributes a near mesh family absent
        # from every normal scene model.  No vehicle/part/material name exception.
        ...
        for entry in sorted(slod_refs, key=str.casefold):
            ...
            parent = str(Path(entry.replace("\\", "/")).parent).casefold()
            # Compare within the same scene directory so a generic mesh-family name in an
            # unrelated part cannot suppress a required SLOD supplement elsewhere.
            unique = {
        ...
        # Every selected SLOD must be proven reachable through the rewritten carbin.  Count-only
        # validation is insufficient because duplicate references could otherwise hide a missed file.
        ...
        if rewritten_entries != selected_cf:
            ...
            raise NearLodNormalizationError(
                "SLOD supplement path rewrite did not cover every selected modelbin: "
                + ", ".join(missing)
            )
                ...
                for info in src.infolist():
                    ...
                    if original_name in parsed and "__slod" not in entry_cf:
                        ...
                        if actual != slod_allowed[original_name]:
                            raise NearLodNormalizationError(
                                f"SLOD family filter mismatch for {original_name}"
                            )
    ...
    report = NearLodNormalizationReport(
        ...
        slod_supplement_families=tuple(sorted(f"{parent}|{family}" for parent, family in supplemented)),
    ...
    if progress:
        progress(
            "M6.23 near-LOD normalization PASS: "
            f"{patched_flags} LOD0-specific mesh flag(s) exposed to the pinned converter; "
            f"{len(supplemented)} directory-scoped SLOD family supplement(s)."
        )
