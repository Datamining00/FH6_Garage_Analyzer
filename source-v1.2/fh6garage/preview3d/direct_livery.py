...
from .kfps_render_backend import ensure_runtime
from .vehicle_index import VehicleAsset as IndexedVehicleAsset
...
class DirectLiveryTextures:
    car_id: int
    paint: np.ndarray  # uint8 [paint_h, paint_w, 4], KFPS/PNG row order (row 0 = texture v=0)
    mask_pages: np.ndarray  # uint8 [3, 1024, 2048, 4], original FH6/KFPS mask resolution
    source_regions: np.ndarray  # float32 [11,4]
    paint_regions: np.ndarray  # float32 [11,4]
    projection_axes: np.ndarray  # float32 [11,4] => axis_x, axis_y, scale_x, scale_y
    projection_mask_regions: np.ndarray  # float32 [11,4]
    section_facings: np.ndarray  # float32 [11,3], sourced from pinned render contract
...
def _archive_masks_exact(archive: Path, contract) -> dict[str, tuple[Image.Image, dict, str]]:
    ...
    This bypasses any upstream presentation/cropping behavior in _archive_masks.
    The swatchbin header is authoritative for width/height and the decoded BC4
    payload is returned at that exact size.  No resizing, padding, or vehicle-
    specific dimensions are guessed.
    """
    try:
        with zipfile.ZipFile(archive) as bundle:
            ...
            for slot in sorted(set(contract.SECTION_TO_SLOT.values())):
                ...
                start = int(texture["payload_offset"]); size = int(texture["payload_size"])
                values = contract._decode_unsigned_bc4(data[start:start + size], width, height)
                if tuple(values.shape) != (height, width):
                    raise DirectLiveryError(
                        f"{source_name} decoded to {tuple(values.shape)}; header declares {(height, width)}."
                    )
...
def _open_trusted_render(path: Path):
    """Open a locally generated section PNG without pre-decoding a second copy."""
...
def _scaled_warped_uv_layer(
    ...
    *,
    ...
    """Resolution-scaled equivalent of the audited KFPS affine warp.

    Projection origins and bounds are stored in native 2048x1024 pixel units.
    M6.23B scales those pixel-space quantities by the same integer factor as the
    section renderer. Normal/1x still calls the pinned implementation directly.
    """
    if scale == 1:
    ...
    if artwork.size != (target_w, target_h):
        raise DirectLiveryError(
            f"{slot} high-resolution artwork is {artwork.size}; expected {(target_w, target_h)}."
        )
    ...
    try:
        ...
        raise DirectLiveryError(
            "Pinned KFPS render contract does not expose the audited slot-orientation sets."
        ) from exc
...
def _scaled_warped_uv_tile(
    ...
    *,
    ...
    """Render only the required warped crop instead of a second full canvas.

    Pillow affine transforms map output coordinates back into source coordinates.
    Cropping a full transformed image at (left, top) is therefore exactly
    equivalent to shifting the affine translation by A*(left, top) and rendering
    only the crop-sized output. M6.24B uses this identity for scale > 1 to avoid
    another 512 MiB/2 GiB full-canvas allocation at 8x/16x.
    """
    ...
    *,
    ...
    for item in ordered:
        ...
        if image.width + padding * 2 > atlas_width:
            raise DirectLiveryError(
                f"The {item['section']} high-resolution paint region is too wide "
                f"for the {atlas_width}px local texture atlas."
            )
    ...
    if height > max_height:
        raise DirectLiveryError(
            f"The high-resolution local section paint atlas would be "
            f"{atlas_width} x {height} pixels; maximum is {max_height}px high."
        )
...
def build_direct_livery_textures(render_result, asset: IndexedVehicleAsset) -> DirectLiveryTextures:
    ...
    M6.23B preserves the KFPS render contract at 1x and applies an audited
    integer-scaled equivalent at 2x. Each section is warped with the per-car
    projection origin/orientation, cropped to the exact source rectangle, packed
    into one paint atlas, and paired with the three RGBA mask pages.
    """
    if int(render_result.car_id) != int(asset.car_id):
        raise DirectLiveryError(
            f"C_livery targets Car ID {render_result.car_id}, but the selected vehicle is Car ID {asset.car_id}."
        )
    ...
    try:
        render_resolution = resolve_livery_resolution(
            getattr(render_result, "resolution_name", "normal")
        )
    except ValueError as exc:
        raise DirectLiveryError(str(exc)) from exc
    expected_canvas = tuple(
        int(v) for v in getattr(render_result, "canvas_size", NATIVE_CANVAS_SIZE)
    )
    if expected_canvas != render_resolution.canvas_size:
        raise DirectLiveryError(
            f"Render result canvas {expected_canvas} does not match "
            f"resolution {render_resolution.label}."
        )
    scale = int(render_resolution.scale)

    vehicle_assets, contract = _load_contract_backend()
    stat = archive.stat()
    runtime_asset = vehicle_assets.VehicleAsset(
    ...
    # M6.16: decode the mask archive directly from the game ZIP. The swatchbin
    # header is authoritative; upstream helper presentation/cropping is not used.
    ...
    if (atlas_w, atlas_h) != NATIVE_CANVAS_SIZE:
        raise DirectLiveryError(
            f"Pinned KFPS native atlas changed to {(atlas_w, atlas_h)}; "
            f"M6.23B scaling was audited for {NATIVE_CANVAS_SIZE}."
        )
    ...
    # M6.8: initialize projection uniforms before the section loop.  M6.8
    # created these arrays after the loop even though valid sections write to
    # them while their paint tiles are being built, causing UnboundLocalError.
    ...
    for index, section in enumerate(SECTION_NAMES):
        ...
        if mask.size != (atlas_w, atlas_h):
            raise DirectLiveryError(
                f"{section} vehicle mask is {mask.size}; expected {(atlas_w, atlas_h)}."
            )
        ...
        try:
            native_source_bounds = tuple(
                int(v) for v in contract._projection_pixel_bounds(projection)
            )
            ...
            with _open_trusted_render(png_path) as artwork:
                if artwork.size != (target_w, target_h):
                    raise DirectLiveryError(
                        f"{section} artwork is {artwork.size}; expected {(target_w, target_h)}."
                    )
                tile = _scaled_warped_uv_tile(
                    artwork,
                    slot,
                    projection,
                    source_bounds,
        ...
        native_mask_crop = mask_values[
            native_source_bounds[1]:native_source_bounds[3],
            native_source_bounds[0]:native_source_bounds[2],
        ]
        if scale == 1:
            mask_crop = native_mask_crop
        else:
            mask_crop = np.asarray(
                Image.fromarray(native_mask_crop, mode="L").resize(
    ...
    try:
        paint_atlas, placements = _pack_paint_tiles_scaled(
            paint_tiles, scale=scale, contract=contract
        )
    except Exception as exc:
    ...
    for record in pending:
        ...
        paint_regions[index] = (
            px / paint_atlas.width,
            py / paint_atlas.height,
            (px + width) / paint_atlas.width,
            (py + height) / paint_atlas.height,
        )

    # IMPORTANT: do not flip rows here.  KFPS loads PNG textures with flipY=false.
    # glTexImage2D/3D receives PIL row 0 as texture row v=0, which reproduces that
    # contract.  M6.0-M6.3 used np.flipud(), inverting the vertical coordinate.
    ...
    return DirectLiveryTextures(
        car_id=int(asset.car_id),
        paint=paint,
        mask_pages=pages,
        source_regions=np.ascontiguousarray(source_regions),
        paint_regions=np.ascontiguousarray(paint_regions),
        projection_axes=np.ascontiguousarray(projection_axes),
        projection_mask_regions=np.ascontiguousarray(projection_mask_regions),
        section_facings=np.ascontiguousarray(section_facings),
        valid_slots=tuple(valid),
        source_names=tuple(sources),
        visible_pixels=tuple(visible_pixels),
        paint_size=(int(paint_atlas.width), int(paint_atlas.height)),
