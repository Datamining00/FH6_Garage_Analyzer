...
from PySide6.QtWidgets import QFrame, QLabel, QLayout, QMainWindow, QWidget
...
def _metadata_labels(card: QWidget) -> tuple[QLabel | None, QLabel | None, QLabel | None]:
    vehicle = None
    title = None
    creator = None
    ...
    for label in card.findChildren(QLabel):
        ...
        if not hasattr(label, "copy_value"):
            continue
        if prefix == vehicle_prefix:
            vehicle = label
        elif prefix == title_prefix:
            title = label
        elif prefix == creator_prefix:
            creator = label
