...
def _set_enabled(window: Any, enabled: bool) -> None:
    ...
    if isinstance(button, QPushButton):
        ...
        button.setText(_txt("성능 측정 켜짐", "Profiling ON") if enabled else _txt("성능 측정 꺼짐", "Profiling OFF"))
        button.blockSignals(False)
    label = getattr(window, "performance_status_label", None)
    if isinstance(label, QLabel):
        label.setText(
            _txt(
                "런타임 측정 중 · 초기 실행 측정은 켜짐 여부와 관계없이 항상 기록됩니다.",
...
def _performance_page(window: Any) -> QWidget:
    ...
    subtitle = QLabel(
        _txt(
            "초기 실행은 매번 자동 측정합니다. 켜기와 끄기는 실행 후 런타임 측정에만 적용됩니다.",
