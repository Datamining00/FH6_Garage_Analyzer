...
def _open_alias_dialog(window: Any) -> None:
    dialog = QDialog(window)
    dialog.setWindowTitle(_txt("제작자 이름 관리", "Creator name manager"))
    dialog.setModal(True)
    ...
    explanation = QLabel(
        _txt(
            "같은 제작자의 이전 이름을 현재 이름으로 통합합니다. 예: A→B, B→C이면 C (B, A). FH6 원본 파일은 수정하지 않습니다.",
            "Merge old names into the creator's current name. Example: A→B then B→C becomes C (B, A). FH6 files are not modified.",
        )
    )
    ...
    form.addWidget(QLabel(_txt("기존/이전 이름", "Existing/old name")), 0, 0)
    source = QComboBox()
    source.setEditable(True)
    form.addWidget(source, 0, 1)
    form.addWidget(QLabel(_txt("현재/새 이름", "Current/new name")), 1, 0)
    target = QComboBox()
    target.setEditable(True)
    form.addWidget(target, 1, 1)
    merge_button = QPushButton(_txt("통합", "Merge"))
    merge_button.setObjectName("primary")
    split_button = QPushButton(_txt("선택 이름 분리", "Split selected name"))
    split_button.setObjectName("secondary")
    form.addWidget(merge_button, 0, 2, 2, 1)
    form.addWidget(split_button, 0, 3, 2, 1)
    ...
    bottom = QHBoxLayout()
    reset_button = QPushButton(_txt("이름 매칭 초기화", "Reset name matching"))
    ...
    def rebuild() -> None:
        ...
        for group in groups:
            ...
            current_item.setData(Qt.ItemDataRole.UserRole, group.current)
            table.setItem(row, 0, current_item)
            table.setItem(row, 1, QTableWidgetItem(", ".join(group.previous)))
            table.setItem(row, 2, QTableWidgetItem(str(len(group.all_names()))))

    def merge_names() -> None:
        ...
        observed = {name.casefold() for name in _observed_creator_names(window)}
        different_group = new_group is not None and old_group is not new_group
        if new.casefold() in observed or different_group:
            answer = QMessageBox.question(
                dialog,
                _txt("제작자 통합 확인", "Confirm creator merge"),
                _txt(
                    f"'{new}' 이름이 이미 존재합니다. '{old}'가 속한 그룹 전체와 '{new}' 그룹을 동일 제작자로 통합하시겠습니까?",
                    f"'{new}' already exists. Merge the entire group containing '{old}' with the '{new}' group?",
                ),
    ...
    def split_name() -> None:
        name = source.currentText().strip()
        if not name:
            return
        if not window.creator_aliases.split(name):
            QMessageBox.information(
                dialog,
    ...
    def reset_aliases() -> None:
        answer = QMessageBox.question(
            dialog,
            _txt("이름 매칭 초기화", "Reset name matching"),
            _txt(
        ...
        try:
            ...
            QMessageBox.warning(dialog, _txt("초기화 실패", "Reset failed"), str(exc))
            return
        _refresh_alias_views(window)
        rebuild()
        message = _txt("이름 매칭을 초기화했습니다.", "Creator name matching was reset.")
        if backup is not None:
            message += "\n\n" + _txt("백업: ", "Backup: ") + str(backup)
        QMessageBox.information(dialog, _txt("초기화 완료", "Reset complete"), message)
