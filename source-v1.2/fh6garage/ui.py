...
APP_STYLE = """
QToolTip { background-color:#ffffff; color:#303341; border:1px solid #dfe1e8;
    padding:4px 6px; }
...
def _classification_toggle_icon(kind: str) -> QIcon:
    ...
    card_assets = {
        "check": ("circle", "#39e75f"),
        "triangle": ("triangle", "#ffe600"),
        "excluded": ("excluded", "#ff4d5a"),
    }
    if kind in card_assets:
        asset, active_color = card_assets[kind]
        return card_toggle_icon(asset, on_color=active_color)
    icon = QIcon()
    icon.addPixmap(
...
class ZoomableImageView(QGraphicsView):
    ...
    def __init__(self, pixmap: QPixmap, parent: Optional[QWidget] = None):
        ...
        self._initial_fit_timer.timeout.connect(self.actual_size)
...
class MainWindow(QMainWindow):
    ...
    def _scan_finished(self, result: ScanResult) -> None:
        try:
            ...
            if controller is not None:
                controller.main_scan_committed()
        finally:
            self._end_busy()
    ...
    def _reset_game_navigation_sessions(self) -> None:
        ...
        for content_type in ("livery", "tuning"):
            ...
            sessions[content_type] = GameGridSession(
                ...
                if content_type != 'livery' or self._content_annotation_key(content_type, record) not in
                getattr(getattr(self, '_fh6_navigation_watch', None), 'removed', set())
            )
    ...
    def _request_game_navigation(
        ...
        if self._game_navigation_pending:
            ...
            return
        if content_type == 'livery' and not getattr(self, '_fh6_navigation_prechecked', False):
            from .navigation_watch import controller
            def resume():
                self._fh6_navigation_prechecked = True
                try:
                    self._request_game_navigation(content_type, key)
                finally:
                    self._fh6_navigation_prechecked = False
            controller(self).guard(resume)
            return
        ...
        if record is not None and content_type == 'livery' and record_locked(self, record):
            self._show_status('잠긴 리버리는 이동할 수 없습니다.', 4000)
            return
    ...
    def _execute_game_navigation(
        ...
        if generation != self._game_navigation_generation:
            ...
            return
        self._game_navigation_pending = False
        if content_type == 'livery' and not _checked:
            from .navigation_watch import controller
            controller(self).guard(lambda: self._execute_game_navigation(
                content_type, key, [], mode, self._game_navigation_generation,
                auto_activate, arrow_interval_ms, True))
            return
        from .backup_policies import record_locked
        record = self._record_for_content_key(content_type, key)
        if record is not None and content_type == 'livery' and record_locked(self, record):
            self._show_status('잠금이 변경되어 이동을 취소했습니다.', 4000)
            return
        session = self._game_navigation_sessions.get(content_type)
        ...
        if content_type == 'livery':
            from .navigation_watch import controller
            watcher = controller(self)
            watcher.target = None
            watcher.timer.stop()
            if deleted and record is not None:
                watcher.start(key, record)
    ...
    def _make_saved_content_card(
        ...
        if content_type == "livery":
            ...
            card_action_grid.addWidget(excluded_box, 5, 1, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignBottom)
        ...
        # Stable two-by-two metadata area:
        # vehicle | acquisition placeholder
        # creator | title
        ...
        vehicle.setStyleSheet(
            "QLabel { background:transparent; color:#171924; border:0; padding:0 2px; "
            "font-size:10.5pt; font-weight:600; }"
        )
        vehicle.setFixedHeight(24)
        ...
        metadata_grid.setRowMinimumHeight(0, 24)
        metadata_grid.setRowMinimumHeight(1, 24)
        metadata_grid.addWidget(vehicle, 0, 0)

        acquisition_box = QLabel("")
        acquisition_box.setObjectName("fh6AcquisitionPlaceholder")
        acquisition_box.setFixedHeight(24)
        acquisition_box.setMinimumWidth(0)
        acquisition_box.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Fixed)
        acquisition_box.setStyleSheet(
            "QLabel { background:transparent; color:#171924; border:0; padding:0 2px; "
            "font-size:10.5pt; font-weight:600; }"
        )
        metadata_grid.addWidget(acquisition_box, 0, 1)
        ...
        metadata_grid.addWidget(title_box, 1, 1)
        ...
        metadata_grid.addWidget(creator_box, 1, 0)
        outer.addLayout(metadata_grid)
        ...
        card._fh6_metadata_grid = metadata_grid
        card._fh6_acquisition_placeholder = acquisition_box
    ...
    def _refresh_after_annotation_change(
        ...
        *,
        ...
        """
        refresher = getattr(self, "_fh6_refresh_filter_counts", None)
        if callable(refresher):
            refresher(content_type)
        filter_box = (
            self.livery_check_filter
            if content_type == "livery"
            else self.tuning_check_filter
    ...
    def _show_livery_image(
        ...
        self._apply_pointing_cursors(dialog)
        from .preview_position import queue_preview_center
        queue_preview_center(dialog, self)
        dialog.exec()
