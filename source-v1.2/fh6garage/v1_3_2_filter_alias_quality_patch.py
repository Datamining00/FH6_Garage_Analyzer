...
def _filter_counts(window: Any, content_type: str) -> dict[int, int]:
    ...
    counts = {mode: 0 for mode in (1, 3, 4, 5, 7, 9, 10, 11, 12, 13, 15)}
    ...
    for card in cards:
        ...
        counts[15] += int(bool(card.property("fh6MoveLocked")))
...
def _refresh_filter_labels(window: Any, content_type: str) -> None:
    ...
    if content_type == "livery":
        from .v1_3_4_livery_backup_filter_patch import _update_filter_count
