...
_PATCH_FLAG = "_fh6assistant_section_boundary_fix_v2"
...
def _build_livery_sections_boundary_safe(decoder: Any, body: bytes, counts: list[int]):
    """Pinned KFPS section walker with a conservative populated-boundary fix.

    KFPS normally advances by LIVERY_POPULATED_REMNANT_SIZE after every populated
    section. Some FH6 C_livery streams start the next populated section directly
    at the current parser position. If an exact 32-byte placement is proven at
    that boundary, preserve it instead of consuming the legacy 18-byte remnant.
    """
    warnings: list[str] = []
...
def _bump_render_cache_revision() -> None:
    try:
        ...
        quality_pipeline.CACHE_VERSION = "v14-quality-pipeline-r6-parser-boundary"
        tiled_quality.CACHE_VERSION = "v14-tiled-quality-r7-parser-boundary"
...
def apply_livery_section_boundary_fix_patch() -> None:
    """Install the conservative populated-to-populated section boundary fix."""
