...
def _entry_card(window: Any, key: str, entry: LiverySnapshotEntry, card_width: int) -> QWidget:
    from .recent_cards import make_card
    return make_card(window, key, entry, card_width)
...
def _open_grouped_change_dialog(window: Any) -> None:
    ...
    def update_changes():
        groups.update(_categorized_changes(window, window._fh6_latest_livery_diff))
        for key, button in buttons.items():
            button.setText(_section_text(key, len(groups[key])))
            button.setEnabled(bool(groups[key]))
        render(force=True)

    dialog._fh6_update_changes = update_changes
