...
def _set_livery_lock(window: Any, card: QWidget, key: str, locked: bool, *, persist: bool) -> None:
    ...
    if persist:
        from .thumbnail_marks_ui import update_card, request
        record = getattr(card, '_fh6_option_record', None)
        if record is not None:
            update_card(window, card, record)
        finder = getattr(window, 'findChildren', None)
        for other in finder(QWidget) if callable(finder) else ():
            if other is card or other.property('fh6LockIdentity') != key:
                continue
            other_lock = getattr(other, '_fh6_lock_placeholder_button', None)
            if other_lock is not None:
                other_lock.blockSignals(True)
                other_lock.setChecked(locked)
                other_lock.blockSignals(False)
                _set_livery_lock(window, other, key, locked, persist=False)
            other_record = getattr(other, '_fh6_option_record', None)
            if other_record is not None:
                update_card(window, other, other_record)
        from .app_options import load_options
        if load_options().write_thumbnail_marks:
            if record is not None:
                request(window, record=record)
