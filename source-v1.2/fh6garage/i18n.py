...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "navigation.delete_notice": {"ko": "이동은 삭제 완료가 아닙니다. 게임에서 삭제하지 않았다면 다시 이동할 수 있습니다. 메인 목록은 새로고침할 때 반영됩니다.", "en": "Navigation does not confirm deletion. You can navigate again if you did not delete in game. Refresh to update the main list."},
    ...
    "navigation.complete_deleted": {"ko": "{count}회 이동 완료 — 게임에서 실제 삭제 여부를 확인해 주세요 ({window})", "en": "Navigation complete ({count} moves) — check whether the item was actually deleted in game ({window})"},
    ...
    "db.local_download_time": {"ko": "로컬 데이터베이스 다운로드 시각: {value}", "en": "Local database download time: {value}"},
    "db.source_last_modified": {"ko": "\n원본 Last-Modified: {value}", "en": "\nSource Last-Modified: {value}"},
    "db.not_updated_tip": {"ko": "아직 수동 차량 데이터베이스 업데이트를 적용하지 않았습니다.", "en": "No manual vehicle database update has been applied yet."},
    "db.update_title": {"ko": "차량 데이터베이스 업데이트", "en": "Vehicle database update"},
    ...
    "db.updating_busy": {"ko": "차량 데이터베이스를 내려받아 갱신하는 중…", "en": "Downloading and updating the vehicle database…"},
    "db.downloading": {"ko": "차량 데이터베이스 다운로드 중…", "en": "Downloading vehicle database…"},
    "db.update_complete_status": {"ko": "차량 데이터베이스 업데이트 완료 — {count} vehicles", "en": "Vehicle database update complete — {count} vehicles"},
    "db.update_complete_title": {"ko": "차량 데이터베이스 업데이트 완료", "en": "Vehicle database update complete"},
    "db.update_complete_message": {"ko": "{count}개의 Car ID 매핑을 적용했습니다.\n저장 위치: {path}", "en": "Applied {count} Car ID mappings.\nSaved to: {path}"},
    "db.update_failed": {"ko": "차량 데이터베이스 업데이트 실패", "en": "Vehicle database update failed"},
    ...
    "car_db.cache_older_warning": {"ko": "기존 업데이트 데이터베이스가 내장 데이터베이스보다 오래되어 사용하지 않음", "en": "Existing update database is older than the bundled database and was not used"},
    ...
    "car_db.response_too_large": {"ko": "차량 데이터베이스 응답이 예상 크기(1 MiB)를 초과했습니다.", "en": "The vehicle database response exceeded the expected size (1 MiB)."},
    "car_db.download_failed": {"ko": "차량 데이터베이스 다운로드 실패: {error}", "en": "Vehicle database download failed: {error}"},
    "car_db.json_failed": {"ko": "차량 데이터베이스 JSON 파싱 실패: {error}", "en": "Vehicle database JSON parsing failed: {error}"},
    "car_db.too_few": {"ko": "차량 데이터베이스 항목이 {count}개뿐입니다. 불완전한 응답으로 판단하여 적용하지 않았습니다.", "en": "The vehicle database contains only {count} entries. The response appears incomplete and was not applied."},
    "car_db.root_not_object": {"ko": "차량 데이터베이스 최상위 JSON이 object가 아닙니다.", "en": "The top-level vehicle database JSON value is not an object."},
    ...
    "car_db.cache_read_failed": {"ko": "업데이트 데이터베이스를 읽지 못함: {error}", "en": "Could not read update database: {error}"},
    "car_db.cache_format_invalid": {"ko": "업데이트 데이터베이스 형식이 잘못됨", "en": "Update database format is invalid"},
    "car_db.count_mismatch": {"ko": "업데이트 데이터베이스 항목 수 정보가 실제 항목 수와 다름", "en": "Update database count metadata does not match the actual entry count"},
    "car_db.count_invalid": {"ko": "업데이트 데이터베이스 항목 수 정보가 잘못됨", "en": "Update database count metadata is invalid"},
