...
def apply_v1_3_2_responsive_columns_fix(MainWindow: Any) -> None:
    """Restore the responsive 2/3/4-column contract lost in the busy-yield patch."""
    if getattr(MainWindow, "_fh6_v132_responsive_columns_fixed", False):
        return

    # The responsiveness relayout functions call their module globals directly,
    # not MainWindow._layout_visible_grid_cards. Replace those globals as well as
    # the bound methods so every sort/filter/resize path uses the same geometry.
    _responsive._responsive_layout_visible_grid_cards = _dynamic_layout_visible_grid_cards
    _responsive._responsive_sync_grid_card_widths = _dynamic_sync_grid_card_widths
    ...
    MainWindow._fh6_v132_responsive_columns_fixed = True
