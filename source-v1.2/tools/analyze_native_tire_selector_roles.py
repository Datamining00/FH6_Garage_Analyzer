...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Diagnostic-only FH6 native tire selector boundary-role analyzer. "
            "Classifies AABB response without assigning production semantics."
        )
    )
    parser.add_argument("archive", help="Path to native tire_*.zip")
    parser.add_argument("--output", required=True, help="Output JSON report path outside the game library")
    return parser
