...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only FH6 automatic native-tire cross-family diagnostic. "
            "Selects representative stock cars for distinct installed TireModelName "
            "families and immediately runs the full morph/formula/stock-geometry validator."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Exact TireModelName to exclude from automatic candidate selection; repeatable.",
    )
    parser.add_argument(
        "--candidate-limit",
        type=int,
        default=3,
        help="Maximum distinct stock tire families to validate (default: 3).",
    )
    parser.add_argument(
        "--min-unique-families",
        type=int,
        default=2,
        help="Minimum distinct tire families required before geometry validation runs (default: 2).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        required=True,
        help="JSON report path outside the FH6 native tire library.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if report.validation is None:
            print(
                "ERROR: insufficient distinct resolvable stock tire families; full geometry validation was not run",
                file=sys.stderr,
            )
            return 3
        return 0
